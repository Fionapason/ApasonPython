...
%             assignin('base', 'data', O.sensor.mf(1).data)
%             
...
%             save('Saver.mat','data');
        ...
        function O = Interface(arduinoObj, arduinoUno, configuration, names, filename, pcmeasurement)
            ...
            O.timerLog = timer('ExecutionMode', 'fixedSpacing','Period',1.5, 'BusyMode', 'drop');
            ...
            O.saver = timer('ExecutionMode', 'fixedSpacing','Period',20);
            ...
            O.GUI.transmembranePressurePanel.Layout.Row = 2;
...
function startupUF(O)
    ...
    if O.sensor.ls(2).data(end) == 1 && O.startState == 0
...
function checkTanks(O)
    ...
    if O.sensor.ls(3).data(end) == 1 % highest level switch of Feed -D24
        disp('--------')
        disp('PLEASE TURN OFF THE PUMP OUTSIDE')
        disp('--------')
        
    elseif O.sensor.ls(4).data(end) == 0 % middle level switch of Feed -D25
        disp('--------')
        disp('THE FEED TANK IS IN THE MIDDLE - START THE PUMP AGAIN')
        disp('--------')
        
    elseif O.sensor.ls(5).data(end) == 0 % low level switch of Feed -D26
        disp('--------')
        disp('THE FEED TANK IS EMPTY - THE SYSTEM WAS SHUT DOWN')
        O.endSystem
    end
        
    if O.sensor.ls(6).data(end) == 1 % highest level switch of Purge -D27
        disp('--------')
        disp('THE PURGE TANK IS ALMOST FULL - BE READY TO EMPTY IT SOON')
        disp('--------')
        
    elseif O.sensor.ls(7).data(end) == 1 % middle level switch of Purge -D28
        disp('--------')
...
function controlSystem(O)
    ...
    elseif O.sensor.ls(1).data(end) == 1 && O.UFState ~= 4 %highest level switch conducting; water reached highest level
    ...
    elseif O.sensor.ls(2).data(end) == 0 &&  O.UFState ~= 3 && O.UFState ~= 1 %water is lower than the middle level switch
